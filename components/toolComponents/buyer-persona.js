...
const BuyerPersona = ({ onComplete, currentUser }) => {
  ...
  const renderField = (section) => {
    console.log('Rendering field for section', { 
      question: section.question,
      hasOptions: !!section.options,
      currentValue: formData[section.question] 
    });
    ...
    const isLongText = section.question.toLowerCase().includes('situation') ||
                      section.question.toLowerCase().includes('problem') ||
                      section.question.toLowerCase().includes('emotion');
    ...
    return isLongText ? (
      <Textarea
        ...
        placeholder={`Enter ${section.question.toLowerCase()}...`}
      />
    ) : (
      <Input
        type="text"
