...
export const fetchNestedChats = async (agentId, currentUser, setNestedChats, db) => {
  ...
  try {
    ...
    console.log(`[Debug] Found ${snapshot.docs.length} nested chats for agent ${agentId}`);

    const chats = snapshot.docs.map((doc) => {
      const data = doc.data();
      console.log(`[Debug] Chat data for ${doc.id}:`, data);
      return {
        id: doc.id,
        ...data,
      };
    ...
    console.log(`[Debug] Filtered to ${chats.length} non-default chats for agent ${agentId}`);
    ...
    setNestedChats((prev) => {
      ...
      console.log(`[Debug] New nested chats state:`, newState);
      return newState;
    });
  } catch (error) {
