...
// Fetch nested chats
export const fetchNestedChats = async (agentId, currentUser, setNestedChats) => {
  if (!agentId || !currentUser?.authenticationID) {
  ...
  try {
    const chats = await firebaseService.queryCollection('conversationNames', {
      where: [
        ...
        { field: 'isDefault', operator: '==', value: false }
      ]
    });
    ...
    console.error(`[Error] Failed to fetch nested chats:`, error);
  }
};
